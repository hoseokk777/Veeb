...
for (const size of normalSizes) {
  await sharp(svg, { density: 300 }).resize(size, size).png().toFile(pub(`icon-${size}.png`))
...
for (const size of iosSizes) {
  await sharp(svg, { density: 300 }).resize(size, size).png().toFile(pub(`apple-touch-icon-${size}.png`))
  console.log(`apple-touch-icon-${size}.png`)
}
await sharp(svg, { density: 300 }).resize(180, 180).png().toFile(pub('apple-touch-icon.png'))
console.log('apple-touch-icon.png')

// maskable 아이콘 (Android 적응형)
const maskSvg = readFileSync(pub('icon-maskable.svg'))
for (const size of [192, 512]) {
  await sharp(maskSvg, { density: 300 }).resize(size, size).png().toFile(pub(`icon-maskable-${size}.png`))
